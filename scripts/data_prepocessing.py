...
def prepocess_data():
    ...
    df_raw = pd.read_csv('data\opt_training_embeddings.csv', index_col=0)
    df = df_raw.T

    # Remove any inconsistency in the cell name in training data
    meta_train_raw_df = pd.read_csv ('data\opt_meta_train.csv')
    ...
    meta_train_processed_2.to_csv('opt_meta_train_processed.csv')
    
    # Add the cell type next to cell name 
    df_processed = pd.read_csv ('opt_meta_train_processed.csv', index_col=0)
